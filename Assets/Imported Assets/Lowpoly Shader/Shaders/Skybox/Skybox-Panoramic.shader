...
Shader "Amazing Assets/Lowpoly Shader/Skybox/Panoramic" {
Properties {
    _Tint ("Color", Color) = (.5, .5, .5, .5)
...
SubShader {
    ...
    Pass {
        ...
        #define LOWPOLY_SHADER_SKYBOX
        #include "../cginc/Geometry.cginc"

        float4 PixelFragment(v2f i)
        {
            #ifdef _MAPPING_6_FRAMES_LAYOUT
            float2 tc = ToCubeCoords(i.texcoord.xyz, i.layout, i.edgeSize, i.faceXCoordLayouts, i.faceYCoordLayouts, i.faceZCoordLayouts);
#else
            float2 tc = ToRadialCoords(i.texcoord.xyz);
